...
export default async function handler(req, res){
  ...
  if (op === 'create') {
    ...
    // Optional auto-print: uncomment next lines to fetch label immediately
    // const pr = await fetch(SOAP_URL, { method:'POST', headers:{'content-type':'text/xml; charset=utf-8','soapaction':''}, body: buildPrint(creds, akis_sideta, NS_MAP.print) });
    // const prTxt = await pr.text();
    // const b64   = pick(prTxt, 'b64_string');
    // return ok(res, { st_flag, st_title, akis_sideta, akis_sub_sideta: sub, pdf_base64: b64 || '' });
